...
export default function PrintComponent() {
  ...
  function callApi(url) {
    ...
    var url =
      server[urlParams.get("plataforma").toLowerCase()] +
      "/handleConsult" +
      url;
    ...
    axios
      ...
      .then(function (response) {
        if (response.status == 200) {
          ...
          setLabelElement(
            <LabelData consulta={response.consult_id} cliente={name} />,
          );
  ...
  if (printElement) {
    printElement.addEventListener("focusout", function () {
      printElement.focus(); // Refocus the input field
    });
  }
    ...
    <div id="main-container">
      <header className="header-area animated menu-fixed fadeInDown">
        <div className="container">
          ...
          <span>
            Gerador de etiquetas de impressão para conferência logística
          </span>
        </div>
      </header>
